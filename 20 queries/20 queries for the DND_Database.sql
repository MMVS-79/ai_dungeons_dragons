-- 1. Count how many characters belong to each race
SELECT
    r.name AS race_name,
    COUNT(ch.id) AS num_characters
FROM races AS r
LEFT JOIN characters AS ch ON r.id = ch.race_id
GROUP BY r.id, r.name;

-- 2. Count total characters per class along with average health
SELECT
    cl.name AS class_name,
    COUNT(ch.id) AS num_characters,
    AVG(ch.max_health) AS avg_health
FROM classes AS cl
LEFT JOIN characters AS ch ON cl.id = ch.class_id
GROUP BY cl.id, cl.name;

-- 3. Get all characters with their campaign name and account email
SELECT
    ch.name AS character_name,
    c.name AS campaign_name,
    a.email AS account_email
FROM characters AS ch
INNER JOIN campaigns AS c ON ch.campaign_id = c.id
INNER JOIN accounts AS a ON c.account_id = a.id
ORDER BY ch.id;

-- 4. Show all logs with campaign name and account email
SELECT
    l.id AS log_id,
    l.message,
    c.name AS campaign_name,
    a.email AS account_email
FROM logs AS l
INNER JOIN campaigns AS c ON l.campaign_id = c.id
INNER JOIN accounts AS a ON c.account_id = a.id
ORDER BY l.created_at DESC;

-- 5. Find the most common race among characters
SELECT
    r.name AS race_name,
    COUNT(ch.id) AS num_characters
FROM races AS r
INNER JOIN characters AS ch ON r.id = ch.race_id
GROUP BY r.id
ORDER BY COUNT(ch.id) DESC
LIMIT 1;

-- 6. Find campaigns that have more than 3 characters
SELECT
    c.id AS campaign_id,
    c.name AS campaign_name,
    COUNT(ch.id) AS num_characters
FROM campaigns AS c
INNER JOIN characters AS ch ON c.id = ch.campaign_id
GROUP BY c.id, c.name
HAVING COUNT(ch.id) > 3;

-- 7. Average attack and defense for characters grouped by class
SELECT
    cl.name AS class_name,
    AVG(ch.attack) AS avg_attack,
    AVG(ch.defense) AS avg_defense
FROM classes AS cl
INNER JOIN characters AS ch ON cl.id = ch.class_id
GROUP BY cl.id;

-- 8. Count how many weapons of each rarity exist
SELECT
    rarity,
    COUNT(*) AS count
FROM weapons
GROUP BY rarity
ORDER BY rarity DESC;

-- 9. Find top 5 characters with the highest current health
SELECT
    name,
    current_health,
    max_health
FROM characters
ORDER BY current_health DESC
LIMIT 5;

-- 10. Total number of logs per campaign
SELECT
    c.id AS campaign_id,
    c.name AS campaign_name,
    COUNT(l.id) AS log_count
FROM campaigns AS c
LEFT JOIN logs AS l ON c.id = l.campaign_id
GROUP BY c.id, c.name;

-- 11. Characters that have both a weapon and a shield assigned
SELECT
    name,
    weapon_id,
    shield_id
FROM characters
WHERE weapon_id IS NOT NULL AND shield_id IS NOT NULL;

-- 12. Total health bonus from armours per character
SELECT
    ch.name AS character_name,
    SUM(a.health) AS total_armour_health
FROM characters AS ch
INNER JOIN armours AS a ON ch.armour_id = a.id
GROUP BY ch.id;

-- 13. Count of items per stat_modified type
SELECT
    stat_modified,
    COUNT(*) AS num_items
FROM items
GROUP BY stat_modified;

-- 14. List campaigns and number of logs, including campaigns with zero logs
SELECT
    c.name AS campaign_name,
    COUNT(l.id) AS log_count
FROM campaigns AS c
LEFT JOIN logs AS l ON c.id = l.campaign_id
GROUP BY c.id;

-- 15. Characters in campaigns that are currently active
SELECT
    ch.name AS character_name,
    c.name AS campaign_name,
    c.state
FROM characters AS ch
INNER JOIN campaigns AS c ON ch.campaign_id = c.id
WHERE c.state = 'active';

-- 16. Average attack of characters per race
SELECT
    r.name AS race_name,
    AVG(ch.attack) AS avg_attack
FROM races AS r
INNER JOIN characters AS ch ON r.id = ch.race_id
GROUP BY r.id;

-- 17. Show top 3 campaigns with the most logs
SELECT
    c.id AS campaign_id,
    c.name AS campaign_name,
    COUNT(l.id) AS log_count
FROM campaigns AS c
INNER JOIN logs AS l ON c.id = l.campaign_id
GROUP BY c.id, c.name
ORDER BY log_count DESC
LIMIT 3;

-- 18. Characters with health below 50% of max_health
SELECT
    name,
    current_health,
    max_health
FROM characters
WHERE current_health < max_health / 2;

-- 19. Find average attack and defense for characters grouped by race and class (2-level grouping)
SELECT
    r.name AS race_name,
    cl.name AS class_name,
    AVG(ch.attack) AS avg_attack,
    AVG(ch.defense) AS avg_defense
FROM characters AS ch
INNER JOIN races AS r ON ch.race_id = r.id
INNER JOIN classes AS cl ON ch.class_id = cl.id
GROUP BY r.id, cl.id;

-- 20. Count of logs per event_type
SELECT
    event_type,
    COUNT(*) AS count
FROM logs
GROUP BY event_type
ORDER BY count DESC;
