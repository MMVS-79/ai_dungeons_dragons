...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[API Continue] Processing continue action for campaign ${body.campaignId}`,
    );
    ...
    console.log(
      `[API Continue] Result: success=${result.success}, phase=${result.gameState.currentPhase}`,
    );
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to process continue action",
      },
      { status: 500 },
    );
  }
